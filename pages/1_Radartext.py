...
# BANCO DE DADOS BRUTO COMPLETO (20 A 30 PRODUTOS FIXOS)
produtos_gringos = {
    ...
    "ZeniCortex": {"col": "EST", "sym": "🟢", "status": "EXCELENTE", "cor": "#22c55e", "p": "ClickBank", "pais": "UK / CA", "motivo": "Suporte auditivo. Concorrência moderada de afiliados, ótima brecha.", "base": 18000},
    "LeanBliss": {"col": "EST", "sym": "🛡️", "status": "MODERADA", "cor": "#eab308", "p": "Digistore24", "pais": "EUA / UK", "motivo": "Nicho de peso mastigável. Concorrência média ideal para orçamentos médios.", "base": 22000},
...
with c_t10:
    ...
    for k, v in produtos_gringos.items():
        if v["col"] == "T10":
            # Botão Streamlit nativo que força o carregamento imediato
...
if "radar_sel" in st.session_state:
    ...
    with cp2:
        st.metric(label="Buscas Acumuladas no MÊS de Referência", value=f"{v_m:,}")
        st.metric(label="Buscas no DIA ATÉ O MOMENTO ATUAL", value=f"{v_d:,}")
