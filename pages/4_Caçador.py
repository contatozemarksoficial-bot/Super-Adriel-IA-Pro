...
def main():
    # 1. CONFIGURAÇÃO PREMIUM DA INTERFACE SAAS (VISUAL SEGURO COLA NO TETO)
    ...
    # POOL DINÂMICO DE PRODUTOS GRINGOS ATUAIS REAIS
    pool_produtos = [
        ...
        {"nome": "Puravive", "plat": "BuyGoods Network", "term": "TENDÊNCIA EXPLOSIVA", "cor": "#00ff66", "cpc": "$1.50", "obs": "Otimização de tissue adiposo marrom."},
        {"nome": "ProDentim", "plat": "Digistore24 Int.", "term": "LEILÃO LIMPO", "cor": "#ffaa00", "cpc": "$1.20", "obs": "Saúde e reconstituição da flora bucal."}
    ]

    # INICIALIZAÇÃO DE SESSÃO PERMANENTE
    if "produtos_cacador_vivos" not in st.session_state:
        ...
        st.session_state.seed_cacador = 3100
    ...
    # GATILHO QUE CRIA A ANIMAÇÃO E MUTA OS PRODUTOS REALMENTE
    if botao_pesquisar:
        with st.spinner("🤖 MONITOR CONECTADO: Efetuando varredura profunda de tráfego Tier 1..."):
            time.sleep(1.0)
            st.session_state.produtos_cacador_vivos = random.sample(pool_produtos, 3)
            st.session_state.seed_cacador = random.randint(2600, 4800)

    # EXTRAÇÃO REATIVA DAS VARIÁVEIS SEGURAS DA CONDIÇÃO DO CLIQUE
    ...
    # 2. COLUNAS EM PARALELO DE 3 PRODUTOS MUTÁVEIS EM TEMPO REAL
    ...
    with c_prod1:
        ...
        st.write(f"**Análise:** Oferta recente mapeada. Menor concorrência de afiliados no leilão fundo de funil gringo para esta estrutura hoje. {selecionados[0]['obs']}")
        st.write("**Melhores Países:** USA, UK, Canadá, Austrália, Alemanha")
        st.write(f"**CPC USA Estimado:** {selecionados[0]['cpc']} | **Outros:** $0.95")
        st.write("")
        df_p1 = pd.DataFrame({"Semanas": ["S1", "S2", "S3", "S4"], "Buscas": [base_seed, int(base_seed * 1.15), int(base_seed * 1.28), int(base_seed * 1.45)]})
    ...
    with c_prod2:
        ...
        st.write(f"**Análise:** Monitoramento contínuo aponta densidade estável de concorrentes. Ideal para tráfego direto ou pre-sell rápida. {selecionados[1]['obs']}")
        ...
        st.write("")
        df_p2 = pd.DataFrame({"Semanas": ["S1", "S2", "S3", "S4"], "Buscas": [int(base_seed * 0.85), int(base_seed * 0.98), int(base_seed * 1.12), int(base_seed * 1.3)]})
    ...
    with c_prod3:
        ...
        st.write(f"**Análise:** Lançamento internacional mapeado com alta comissão liberada de saída pelo produtor no leilão. {selecionados[2]['obs']}")
        st.write("**Melhores Países:** USA, UK, Irlanda, Austrália, Canadá")
        st.write(f"**CPC USA Estimado:** {selecionados[2]['cpc']} | **Outros:** $0.85")
        st.write("")
        df_p3 = pd.DataFrame({"Semanas": ["S1", "S2", "S3", "S4"], "Buscas": [int(base_seed * 1.15), int(base_seed * 1.25), int(base_seed * 1.38), int(base_seed * 1.55)]})
