...
def main():
    ...
    # PASSO 1 - REGISTRO E HOSPITALIDADE
    ...
    # Caixa verde premium de recomendação compliance
    ...
    # INTERFACE DE CUSTOMIZAÇÃO DO PRODUTO (IDÊNTICO AO SEU PRINT)
    ...
    with col_dir:
        ...
        st.write("Termos de politicas obrigatórios exigidos pelos robôs do Google Ads, declarando isenção de responsabilidade de marcas:")
    ...
    with c_c1:
        st.markdown("<h4>🌐 Headline Principal</h4>", unsafe_allow_html=True)
        st.text_area("Copiar Headline:", value="WELCOME TO THE OFFICIAL INBOUND REDIRECTION PORTAL", height=120, key="copy_c1")

    with c_c2:
        st.markdown("<h4>📋 Texto de Demanda</h4>", unsafe_allow_html=True)
    ...
    with c_c3:
        st.markdown("<h4>🎯 Link de Botão CTA</h4>", unsafe_allow_html=True)
    ...
    with c_c4:
        ...
        st.markdown("<h4>🏆 Veredito Legal</h4>", unsafe_allow_html=True)
